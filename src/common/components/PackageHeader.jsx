			...
			<div>
				<nav>
					<ul className="tabs">
						...
						<li className={this.props.activeView === 'diff' ? 'active' : ''}>
							<Link to={`${baseRoute}/diff/${this.props.rhsversion }`}>
								<svg height="20" className="icon" viewBox="0 0 24 24">
...
function mapStateToProps({ packageReducer }, ownProps) {
	const { packageId, version, activeView, ts, rhsversion, rhsts } = packageReducer;
